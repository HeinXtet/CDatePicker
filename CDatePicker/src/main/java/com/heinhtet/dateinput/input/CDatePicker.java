...
import com.heinhtet.dateinput.R;
...
public class CDatePicker extends RelativeLayout {
    ...
    private View daySlashView;
    private View monthSlashView;
    // Properties
    ...
    public CDatePicker(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CDatePicker(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(attrs, R.styleable.CDatePicker,
                defStyleAttr, 0);
        textColor = attributes.getColor(R.styleable.CDatePicker_ldp_text_color, Color.BLACK);
        hintColor = attributes.getColor(R.styleable.CDatePicker_ldp_hint_color, Color.GRAY);
        showFullDate = attributes.getBoolean(R.styleable.CDatePicker_ldp_show_full_date, true);
        labelSlashColor = attributes.getColor(R.styleable.CDatePicker_ldp_slash_color, Color.GRAY);


        int dateFormatValue = attributes.getInteger(R.styleable.CDatePicker_ldp_date_format,
    ...
    private void showFocusView(boolean hasFocus) {
        if (hasFocus) {
            focusLayout.animate().alpha(1).setInterpolator(new AccelerateDecelerateInterpolator()).setDuration(500)
                    .start();
        } else {
            focusLayout.animate().alpha(0).
                    setInterpolator(new AccelerateDecelerateInterpolator()).setDuration(500)
                    .start();
        }
        if (this.focusChangedListener != null) {
            focusChangedListener.onFocusChangedListener(hasFocus);
        } else {
    ...
    private void initView() {
        ...
        daySlashView = findViewById(R.id.day_slash_view);
        monthSlashView = findViewById(R.id.month_slash_view);


        daySlashView.setBackgroundColor(labelSlashColor);
        monthSlashView.setBackgroundColor(labelSlashColor);
    ...
    public interface onFocusChangedListener {
        void onFocusChangedListener(Boolean hasFocus);
    }
